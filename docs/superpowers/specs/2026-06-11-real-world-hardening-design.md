...
emission honors `valuePrefix` — `headers[name] = "<prefix>" + value` (baked
literal; empty prefix → byte-identical to today's emission, keeping existing
goldens stable). Env pickup/REQUIRED_ENV_VARS derive from the synthesized
...
- **Content sniffing** (when `--surface` absent): parse as YAML/JSON → top-level
  `openapi:` → `rest` + `application/openapi+yaml|json`; `swagger: "2.0"` →
  `rest` + `application/swagger+*`; GraphQL SDL heuristic (`type Query`/
  `schema {`) → `graphql` + `application/graphql`; else `docs` + served
  content-type. `--surface` overrides.
...
- **Fern stamping** (`src/renderers/fern-oas-rewrite.ts`): dotted paths emit as
  `$response.data.results` — Fern's documented syntax already supports
  dot-delimited response paths (the CLI zod schema validates
  `$response.`-prefixed dot-delimited strings — re-verify acceptance with the
  pure-Node config tests; Docker regen stays in CI).
...
`{data: {results: [], total, per_page, page}}` + `page`/`per_page` int params)
...
next_cursor}}`) so the chain is golden-locked end-to-end (one reviewed regen).
...
2. `add-source` round-trips the dogfood flow: `skillship add-source
   ...
   with a rest source and gold coverage.
