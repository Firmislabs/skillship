...
- [ ] **Step 1: Failing tests** (source assertions: generator emits the guard `if (next === prev) return;`, the three stop conditions, `*Pages` method + the `./pagination.js` import appear in resources output only for planned ops, absent otherwise — assert the no-plan case emits resources byte-identical to before by snapshotting the pre-change emission string in the test).
- [ ] **Step 2: Run → FAIL.**
- [ ] **Step 3: Implement.**
