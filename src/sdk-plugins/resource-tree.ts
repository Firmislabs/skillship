...
import {
  buildOpByLeaf,
  emitAttachResources,
  emitRequestOptsInterface,
  emitResourceTreeInterface,
} from "./resource-tree-pages.js";
...
export function generateResourceTreeModule(
  ...
  const opByLeaf = buildOpByLeaf(resolveAssignments(ops, overlay));
  ...
  if (hasPlan) lines.push('import { paginate } from "./pagination.js";');
  lines.push(...emitResourceTreeInterface(tree, opByLeaf, plans));
  lines.push(...emitRequestOptsInterface());
  lines.push(...emitAttachResources(tree, opByLeaf, plans));
  return lines.join("\n") + "\n";
}
