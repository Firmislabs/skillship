...
function runDescribe(args: Record<string, unknown>): ToolResult {
  ...
  lines.push("Auth: set GQL_EXAMPLE_TOKEN.");
  lines.push("Base URL: not baked in — set GQL_EXAMPLE_BASE_URL.");
  if (a.destructive) {
...
function resolveBaseUrl(
  ...
  return env["GQL_EXAMPLE_BASE_URL"] ?? null;
}
...
async function runInvoke(
  ...
  if (
    ...
    !confirm &&
    env["GQL_EXAMPLE_MCP_ALLOW_DESTRUCTIVE"] !== "1"
  ) {
    return text(
  ...
  if (baseUrl === null) {
    return text("base_url_not_set: set GQL_EXAMPLE_BASE_URL.", true);
  }
  try {
