...
function runDescribe(args: Record<string, unknown>): ToolResult {
  ...
  lines.push("Auth: set MIN_EXAMPLE_TOKEN.");
  lines.push(
    "Base URL: https://api.example.com/v1 (override with MIN_EXAMPLE_BASE_URL)",
  );
  if (a.destructive) {
...
function resolveBaseUrl(
  ...
  return env["MIN_EXAMPLE_BASE_URL"] ?? "https://api.example.com/v1";
}
...
async function runInvoke(
  ...
  if (
    ...
    !confirm &&
    env["MIN_EXAMPLE_MCP_ALLOW_DESTRUCTIVE"] !== "1"
  ) {
    return text(
  ...
  if (baseUrl === null) {
    return text("base_url_not_set: set MIN_EXAMPLE_BASE_URL.", true);
  }
  try {
